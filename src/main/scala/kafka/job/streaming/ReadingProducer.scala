...
class StreamingJobExecutor_1(spark: SparkSession, KafkaReaderConfig_1: KafkaReaderConfig_1, checkpointLocation: String, JDBCConfig_1: JDBCConfig_1) {
  ...
  def execute(): Unit = {
    ...
    val transformDF_2 = transformDF_1.select(
      ...
      split(col("value"), ",").getItem(4).as("location"),
      split(col("value"), ",").getItem(5).as("country"),
      split(col("value"), ",").getItem(6).as("latitude"),
      split(col("value"), ",").getItem(7).as("longitude"),
      split(col("value"), ",").getItem(8).as("airport_code"),
      split(col("value"), ",").getItem(9).as("airport_name"),
      split(col("value"), ",").getItem(10).as("injury_severity"),
      split(col("value"), ",").getItem(11).as("aircraft_damage"),
      split(col("value"), ",").getItem(12).as("aircraft_category"),
      split(col("value"), ",").getItem(13).as("registration_number"),
      split(col("value"), ",").getItem(14).as("make"),
      split(col("value"), ",").getItem(15).as("model"),
      split(col("value"), ",").getItem(16).as("amateur_built"),
      split(col("value"), ",").getItem(17).as("number_of_engines"),
      split(col("value"), ",").getItem(18).as("engine_type"),
      split(col("value"), ",").getItem(19).as("far_description"),
      split(col("value"), ",").getItem(20).as("schedule"),
      split(col("value"), ",").getItem(21).as("purpose_of_flight"),
      split(col("value"), ",").getItem(22).as("air_carrier"),
      split(col("value"), ",").getItem(23).as("total_fatal_injuries"),
      split(col("value"), ",").getItem(24).as("total_serious_injuries"),
      split(col("value"), ",").getItem(25).as("total_minor_injuries"),
      split(col("value"), ",").getItem(26).as("total_uninjured"),
      split(col("value"), ",").getItem(27).as("weather_condition"),
      split(col("value"), ",").getItem(28).as("broad_phase_of_flight"),
      split(col("value"), ",").getItem(29).as("report_status"),
      split(col("value"), ",").getItem(30).as("duplication_date")
    )
